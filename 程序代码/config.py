...
RESULTS_ROOT = WORKSPACE_ROOT / "实验结果"
BASE_RESULTS_ROOT = RESULTS_ROOT / "通道4六分类_基础结果"
FINAL_RESULTS_ROOT = RESULTS_ROOT / "修正倍频_六分类最终结果"
...
class PipelineConfig:
    ...
    output_root: Path = field(
        default_factory=lambda: BASE_RESULTS_ROOT
    )
    channel: int = 4
...
class ModelingConfig:
    ...
    output_root: Path = field(
        default_factory=lambda: BASE_RESULTS_ROOT / "modeling"
    )
    random_state: int = 42
    correlation_threshold: float = 0.90
    near_zero_variance_threshold: float = 1e-12
