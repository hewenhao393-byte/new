...
class FeatureFilterResult:
    ...
    retained_high_correlation_pairs: list[tuple[str, str, float]] = field(default_factory=list)
...
def fit_training_feature_filter(
    ...
    protected = set(config.protected_feature_columns if protected_features is None else protected_features)
    ...
    retained_high_corr_pairs: list[tuple[str, str, float]] = []
    while True:
        pairs = [
        ...
        if not pairs:
            break
        removable_pairs = [pair for pair in pairs if not (pair[0] in protected and pair[1] in protected)]
        if not removable_pairs:
            retained_high_corr_pairs.extend(pairs)
            break
        left, right, _ = max(removable_pairs, key=lambda item: item[2])
        to_remove = _pick_correlated_feature_to_remove(left, right, config)
        if to_remove in protected:
            other = right if to_remove == left else left
            if other not in protected:
                to_remove = other
            else:
                retained_high_corr_pairs.append((left, right, float(corr.loc[left, right])))
                break
    ...
    return FeatureFilterResult(
        ...
        retained_high_correlation_pairs=retained_high_corr_pairs,
    )
