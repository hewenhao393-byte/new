...
def test_default_output_paths_use_chinese_results_directory() -> None:
    pipeline = PipelineConfig()
    modeling = ModelingConfig()

    assert pipeline.output_root.name == "通道4六分类_基础结果"
    assert pipeline.output_root.parent.name == "实验结果"
    assert modeling.feature_root == pipeline.output_root
    assert modeling.output_root == pipeline.output_root / "modeling"
    assert "程序代码/outputs" not in pipeline.output_root.as_posix()
