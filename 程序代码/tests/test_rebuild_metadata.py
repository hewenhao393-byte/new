...
def test_condition_catalog_maps_chinese_faults_to_actual_rpm(tmp_path: Path) -> None:
    ...
    assert catalog.nominal_rpm_for("Motor-4", 70) == 2070
    assert catalog.nominal_rpm_for("Motor-2", 75) == 1110


def test_build_file_index_uses_nominal_speed_rpm_by_default(tmp_path: Path) -> None:
    root = tmp_path / "Vibration"
    path = root / "Motor-4" / "70" / "正常状态1" / "振动_电机4_70_时域-正常状态1-通道4.csv"
    _write_trace_csv(path)
    ...
    config = PipelineConfig(data_root=root, output_root=tmp_path / "output")
    index = build_file_index(config, catalog=ConditionCatalog.from_workbook(workbook))

    assert index["rpm"].tolist() == [2070.0, 2070.0]
    assert index["speed_percent"].tolist() == [70, 70]


def test_condition_catalog_reads_current_workbook_format_and_string_severity(tmp_path: Path) -> None:
    workbook = tmp_path / "current_conditions.xlsx"
    rows = [
        ["Measurements"],
        ["Order", "Setup", "Failure description", "Severity", "Speed (%)", "Speed (RPM) +- 5"],
        [1, "motor 4", "Healthy", "N/A", 0.7, 2070],
        [2, "motor 4", "Coupling", "2D", 0.7, 2070],
    ]
    with pd.ExcelWriter(workbook) as writer:
        pd.DataFrame(rows).to_excel(writer, sheet_name="Ordered Measurements", index=False, header=False)
    ...
    assert catalog.rpm_for("Motor-4", 70, "正常状态1") == 2070
    assert catalog.rpm_for("Motor-4", 70, "联轴器故障2D") == 2070
