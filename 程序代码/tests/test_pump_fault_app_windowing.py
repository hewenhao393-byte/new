...
def test_segmenter_uses_formal_window_size_and_step() -> None:
    record = _preprocessed_record(np.arange(9600, dtype=np.float64))

    result = segment_preprocessed_signal(record)

    assert result.window_size == 4800
    assert result.step_size == 2400
...
def test_segmenter_produces_expected_window_boundaries() -> None:
    ...
    assert [window.start_index for window in result.windows] == [0, 2400, 4800]
    assert [window.end_index for window in result.windows] == [4800, 7200, 9600]
...
def test_segmenter_preserves_window_samples() -> None:
    ...
    assert np.array_equal(result.windows[0].samples, samples[0:4800])
    assert np.array_equal(result.windows[1].samples, samples[2400:7200])
    assert np.array_equal(result.windows[2].samples, samples[4800:9600])
...
def test_segmenter_outputs_window_time_ranges() -> None:
    ...
    assert result.windows[0].end_time_seconds == 0.4
    assert result.windows[1].start_time_seconds == 0.2
    assert result.windows[1].end_time_seconds == 0.6
...
def test_segmenter_records_overlap_ratio() -> None:
    ...
    assert result.window_duration_seconds == 0.4
